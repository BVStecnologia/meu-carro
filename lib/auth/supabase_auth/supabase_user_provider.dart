...
class MeuCarroAiSupabaseUser extends BaseAuthUser {
  MeuCarroAiSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> meuCarroAiSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MeuCarroAiSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
